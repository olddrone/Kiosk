...
public class ChoiceTopping extends SwingManager implements SwingManageable {
    User user;
    Order order;
    Menu menu;
    ArrayList<JButton> toppingList = new ArrayList<>();
    JButton b = new JButton("돌아가기");
    JLabel label = new JLabel("추가 토핑선택", JLabel.LEFT);
    Color c=new Color(8,161,64);

    public ChoiceTopping(JFrame f, ClassControl data,Manageable user, Order order, Menu menu) {
        super(f, data);
        this.user = (User) user;
        this.order = order;
        this.menu=menu;
    ...
    void createButton() {
        for (Topping m : data.getToppingList().getmList()) {
    ...
    public void actionPerformed(ActionEvent e) {
        if (e.getSource() == b) {
            ...
            int idx = toppingList.indexOf(e.getSource());
            Topping topping = data.getToppingList().getList(idx);
            changerPanel(new ChoiceVegetable(frame, data, user,order, menu, topping));
        }
    }
